  ...
  *************************************************************/\
  ...
  \
    ...
    if (Serial2.available() > 4)\
      \{  \
          //Serial.println("Decoding preamble");\
          // decode pre-amble and only continue if preamble headed\
          bool preambleRecv = true;\
          for (int i = 0; i < 2; i++) \{\
          ...
          \}\
  \
          if (preambleRecv == false)\{\
            Serial.println("BT: failed to recieve correct preamble ... some data may be missing");\
            int recvPre = 0;\
            while (recvPre < 2) \{\
              if(Serial.read() == 0x55) \{\
                recvPre ++;\
  ...
  \
          ...
          _length[0] = Serial2.read();\
          _length[1] = Serial2.read();\
          length = (_length[0] << 8) & 0xFF00 | _length[1] & 0x00FF;\
  \
          \
  ...
  bool bleTX(String msg) \{\
    if (!Serial2)\
      return false;\
  \
    size_t bytesSent = Serial2.print(msg);\
    if (bytesSent == 0)\
      return false;\
  \
    return true;\
  ...
  void setup() \{\
    ...
    while (!Serial || !Serial2) \{\} // Wait for initilization\
    Serial.println("##########Initialize Serial##########");\
    Serial.println("Serial Initialized\\r\\n");\
  \
    // Initialize peripherals\
    bool bleSuccess = setupBLE();\
    bool ldclSuccess = true;\
    bool motSuccess = true;\
    bool shuntSuccess = true;\
    bool heatCoolSuccess = true;\
    ...
    while (!(bleSuccess && ldclSuccess && motSuccess && shuntSuccess && heatCoolSuccess && thermistorSuccess)) \{\
      if(!bleSuccess)\
  ...
  \
    ...
    if (DEBUG_PRINT) \{\
      ...
      bleTX("Received Command: " + cmdLine);\
