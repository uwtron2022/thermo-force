...
bool setupLDCL () {
  ...
  pinMode(LDCL_DOUT, INPUT_PULLUP);
...
//  }
  ...
  if (DEBUG_PRINT) {
    ...
    Serial.println("Loadcell ISR Bool: " + String(ldclMeasRdy));
...
//  bool motDirCurrent = motDirection;
...
//  if (1.0*(trueDir - FUZZY_DEADBAND) >= 0) {
...
//    Serial.println("Selected Motor Dir (High-CW, Low-CCW): " + String(motDirCurrent) + "\r\n");
//  }
//      
...
//    float massScalar = (1.0*(refMass-MASS_MIN)/(MASS_MAX-MASS_MIN)) + 0.1;
//    float vCtrl = 2.5*(massDelta-massDelta1);
//    if ((vCtrl < 0) && (vCtrl > -MOT_STICTION))
//      vCtrl -= 0.25;
//    else if ((vCtrl > 0) && (vCtrl < MOT_STICTION))
//      vCtrl += 0.25;
...
//    if (DEBUG_PRINT) {
//      Serial.println("***Weight Feedback Signals***");
//      Serial.println("Mass Scalar: " + String(massScalar));
//      Serial.println("Mass Delta: " + String(massDelta-massDelta1));
//      Serial.println("Requested Voltage: " + String(vCtrl));
//      Serial.println("PWM Signal: " + String(vRef) + "\r\n");
//    }
//  }
